...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = f"#تـحـديث_جـديـد\n\nالتغـييـرات:\n`{changelog}`"
...
async def update_bot(event, repo, ups_rem, ac_br):
    ...
    razan = await event.edit(
        "تم التحديث بنجاح ✓\nجارى اعاده تشغيل البوت يرجى الانتظار "
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "يتم البحث على التحديثات ...")
    ...
    if changelog == "" and not force_update:
        await event.edit("التليثون الخاص بك محدث الى اخر اصدار ✓")
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"ارسل `{cmdhd}تحديث الان` لتحديث السورس .")

    if force_update:
        await event.edit("- يتم التحديث الاجباري لأخر اصدار من السورس انتظر قليلا")
    if conf == "الان":
        await event.edit("[■□□□□□□□□□] 10% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■□□□□□□□□] 20% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■□□□□□□□] 30% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■■□□□□□□] 40% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■■■□□□□□] 50% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■■■■□□□□] 60% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■■■■■□□□] 70% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■■■■■■□□] 80% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■■■■■■■□] 90% جارى التحديث")
        await asyncio.sleep(1)
        await edit_or_reply(event, "[■■■■■■■■■■] 100% جارى التحديث")
