...
async def promote(event):
    ...
    if not user:
        return
    jmthonevent = await edit_or_reply(event, "جارى رفعه ادمن ...")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await jmthonevent.edit(NO_PERM)
    await jmthonevent.edit("تم رفعه ادمن ✓")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#رفع_ادمن\
...
async def demote(event):
    ...
    if not user:
        return
    jmthonevent = await edit_or_reply(event, "جارى تنزيله من الادمن ...")
    ...
    try:
        ...
        return await jmthonevent.edit(NO_PERM)
    await jmthonevent.edit("تم تنزيله من الادمن ✓")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#تنزيل_ادمن\
...
async def _ban_person(event):
    ...
    jmthonevent = await edit_or_reply(event, "جارى حظر المستخدم ....")
    try:
...
async def startgmute(event):
    if event.is_private:
        await event.edit("جارى كتم الشخص ....")
...
async def endgmute(event):
    if event.is_private:
        await event.edit("جارى الغاء كتم الشخص ....")
        await asyncio.sleep(2)
        userid = event.chat_id
        reason = event.pattern_match.group(1)
    else:
...
async def kick(event):
    ...
    if not user:
        return
    jmthonevent = await edit_or_reply(event, "جارى طرد المستخدم ....")
    try:
