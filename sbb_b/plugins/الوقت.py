...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        sa3ed = datetime.now(pytz.timezone(f"{T_Z}"))
        HM = sa3ed.strftime("%I:%M")
        for normal in HM:
            if normal in normzltext:
                namefont = namerzfont[normzltext.index(normal)]
                HM = HM.replace(normal, namefont)
        name = f"{RR7PPP} {HM} {RR7PP} "
        LOGS.info(name)
        try:
...
@sbb_b.ar_cmd(pattern="الساعه$")
async def _(event):
    if gvarstatus("autoname") is not None and gvarstatus("autoname") == "true":
        return await edit_delete(event, "الساعه شغاله بالفعل .")
    addgvar("autoname", True)
    await edit_delete(event, "تم تفعيل الساعه ✓")
    await autoname_loop()
...
@sbb_b.ar_cmd(pattern="تعطيل ([\s\S]*)")
async def _(event):
    ...
    if (
        ...
        if gvarstatus("autoname") is not None and gvarstatus("autoname") == "true":
            ...
            return await edit_delete(event, "تم تعطيل الساعه ✓")
        return await edit_delete(event, "الساعه لم تقم بتفعيلها .")
    ...
    if input_str == "وقتي كروب" or input_str == "دردشة وقتية":
        group = get_autogroup()
