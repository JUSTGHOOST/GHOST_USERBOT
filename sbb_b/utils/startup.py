...
LOGS = logging.getLogger("اعداد ايطاليا")
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            ...
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
                        "لا توجد صلاحيات كافية لارسال الرسائل في جروب الحفظ او التخزين"
                    )
                if entity.default_banned_rights.invite_users:
                    LOGS.info(
                        "لا توجد صلاحيات كافية لاضافة الاعضاء في جروب الحفظ او التخزين"
                    )
        except ValueError:
        ...
        print("تم انشاء جروب الحفظ بنجاح")
        flag = True
